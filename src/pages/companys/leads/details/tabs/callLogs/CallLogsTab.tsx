...
const CallLogsTab = () => {
  ...
  const handleSaveLog = (formData: CallLogFormData) => {
    if (editingLog) {
      setCallLogs((prev) =>
        prev.map((log) =>
          log.id === editingLog.id ? { ...log, ...formData } : log,
        ),
      );
    } else {
  ...
  const getCallTypeIcon = (type: string) => {
    switch (type) {
      case "inbound":
        return <PhoneIncoming className="h-3 w-3 mr-1" />;
      case "outbound":
        return <PhoneOutgoing className="h-3 w-3 mr-1" />;
      case "missed":
        return <PhoneMissed className="h-3 w-3 mr-1" />;
      default:
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete the call log for "
              {logToDelete?.subject}". This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
