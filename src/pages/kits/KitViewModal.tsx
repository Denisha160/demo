...
const KitViewModal = ({ open, onClose, kitId, onEdit }: KitViewModalProps) => {
    ...
    return (
        <Modal
            ...
            title={
                isLoading ? "Loading..." : (kit?.name ?? "Kit Details")
            }
            description={
                kit
                ...
                <div className="space-y-5 mt-2">
                    ...
                    <div className="grid grid-cols-3 gap-3">
                        <div className="bg-muted/30 rounded-sm p-3 border border-border/60 space-y-1">
                            ...
                            <Badge variant={kit.is_active ? "success" : "secondary"} className="text-[10px] uppercase font-bold">
                                {kit.is_active ? "Active" : "Inactive"}
                            </Badge>
                        </div>
                        <div className="bg-muted/30 rounded-sm p-3 border border-border/60 space-y-1">
                            <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wide">Components</p>
                            <p className="text-lg font-bold text-foreground leading-none">{totalItems}</p>
                        </div>
                        <div className="bg-primary/5 rounded-sm p-3 border border-primary/10 space-y-1">
                            <p className="text-[10px] font-semibold text-primary/70 uppercase tracking-wide">Kit Price</p>
                                ...
                                <table className="w-full text-xs">
                                    <thead className="bg-muted/40 border-b border-border/60">
                                        <tr>
                                            ...
                                            <th className="px-3 py-2 text-center font-semibold text-muted-foreground w-[80px]">Qty</th>
                                            <th className="px-3 py-2 text-right font-semibold text-muted-foreground w-[100px]">Price</th>
                                        </tr>
                                    </thead>
                                                ...
                                                <td className="px-3 py-2.5">
                                                    <div className="flex items-center gap-2">
                                                        {item.image_url ? (
                                                            <img
                                                                src={item.image_url}
                                                                alt={item.product_name || item.finished_product_name}
                                                        ...
                                                        <span className="font-medium text-foreground">
                                                            {item.product_name || item.finished_product_name || "Unknown Product"}
                                                        </span>
                                                    </div>
                                                </td>
