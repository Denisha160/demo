...
const FragrancesPage = () => {
  ...
  const { mutateAsync: createFragrance, isPending: isCreating } =
    useCreateFragrance();
  const { mutateAsync: updateFragrance, isPending: isUpdating } =
    useUpdateFragrance();
  const { mutate: deleteFragrance, isPending: isDeleting } =
    useDeleteFragrance();
  ...
  const [fragranceToDelete, setFragranceToDelete] = useState<Fragrance | null>(
    null,
  );
  ...
  return (
    <div className="w-full mx-auto space-y-2 animate-fade-in">
      ...
      <AlertDialog
        open={!!fragranceToDelete}
        onOpenChange={(open) => !open && setFragranceToDelete(null)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Are you absolutely sure?</AlertDialogTitle>
            <AlertDialogDescription>
              This will permanently delete the fragrance "
              {fragranceToDelete?.name}". This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
