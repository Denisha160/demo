...
const BatchBomModal = ({
  ...
  const totalCost = useMemo(() => {
    ...
    return bomDetails.raw_materials.reduce(
      (sum, m) => sum + (m.raw_quantity || 0) * (m.cost_price || 0),
      0,
    );
  }, [bomDetails]);
      ...
      <div className="space-y-6">
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-20 space-y-4">
            ...
            <p className="text-xs text-muted-foreground animate-pulse font-medium tracking-wide uppercase">
          ...
          <div className="flex flex-col items-center justify-center py-16 space-y-3 bg-muted/20 border border-dashed rounded-lg">
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-muted-foreground">
                This product doesn't have a configured Bill of Materials.
              </p>
            </div>
          </div>
        ) : (
          <>
                    ...
                    >
                      <td className="px-4 py-3">
                        ...
                        <div className="text-[10px] text-muted-foreground font-mono">
                          #{m.raw_product_id.split("-")[0].toUpperCase()}
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-3 text-center">
                        <Badge
                          variant="outline"
                          className="text-[9px] font-bold rounded-sm bg-muted/30"
                        >
                          {m.raw_unit}
                        </Badge>
                      </td>
                      <td className="px-4 py-3 text-right font-mono font-bold text-primary">
                        ₹
                        {(
                          (m.raw_quantity || 0) * (m.cost_price || 0)
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <div className="p-4 bg-muted/30 rounded-md border border-border space-y-2">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[10px] text-muted-foreground font-medium italic text-emerald-600">
                    Unit Price
                  </span>
                </div>
              </div>
              ...
              >
                ...
                >
                  <TrendingUp
                    className={`h-4 w-4 ${profit < 0 ? "rotate-180" : ""}`}
                  />
                  <span className="text-[10px] font-bold uppercase tracking-wide">
                ...
                <div className="flex flex-col">
                  <span
                    className={`text-lg font-bold font-mono ${profit >= 0 ? "text-emerald-600" : "text-destructive"}`}
                  >
                    {profit >= 0 ? "+" : ""}₹
            ...
            <div className="flex items-center gap-2 p-3 bg-muted/10 rounded-lg border border-border/50">
              ...
              <p className="text-[11px] text-muted-foreground leading-relaxed">
                This recipe defines the standard consumption of raw materials
                for producing{" "}
                <span className="font-bold text-foreground">1 single unit</span>{" "}
                of this finished good. Inventories will be deducted based on the
