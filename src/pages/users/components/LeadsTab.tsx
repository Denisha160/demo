...
import { TrendingUp, IndianRupee, PieChart, BarChart3, FileText } from "lucide-react";
...
const activityColumns: Column<Record<string, unknown>>[] = [
    ...
    { key: "partner", header: "Client / Dealer" },
    {
        ...
        render: (v) => <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-sm ${v.type === "Custom Branding" ? "bg-amber-100 text-amber-700 border border-amber-200" : "bg-slate-100 text-slate-700 border border-slate-200"}`}>{v.type as string}</span>
    },
    { key: "value", header: "Value" },
    {
        key: "status", header: "Status",
        render: (q) => <StatusBadge status={q.status as string} variant={q.status === "Delivered" || q.status === "Dispatched" ? "success" : "info"} />
    },
    ...
    return (
        <div className="space-y-4">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
                <StatCard title="Total Leads" value={userData.totalLeads?.toString() || "0"} change="+12 this week" changeType="positive" icon={<BarChart3 className="h-4 w-4" />} />
                <StatCard title="Conversion" value={userData.conversionRate || "0%"} change="+2.4% vs avg" changeType="positive" icon={<TrendingUp className="h-4 w-4" />} />
                <StatCard title="Avg. Prod Time" value={userData.avgProductionTime || "N/A"} change="Custom Branding" changeType="neutral" icon={<FileText className="h-4 w-4" />} />
                    ...
                    <div className="p-5 border border-border rounded-sm bg-card shadow-sm">
                        ...
                        <div className="flex flex-col md:flex-row items-center gap-8">
                            <ResponsiveContainer width="100%" height={180} className="md:w-1/2">
                                <RePieChart>
                                    <Pie data={leadPipelineData} cx="50%" cy="50%" innerRadius={45} outerRadius={75} dataKey="value" stroke="none" paddingAngle={2}>
                    ...
                    <div className="p-5 border border-border rounded-sm bg-card shadow-sm overflow-hidden relative group">
                        ...
                        <div className="space-y-2">
                            <div className="flex justify-between text-xs mb-1">
                                <span className="font-semibold text-foreground">{userData.revenue || "₹0"} achieved</span>
                                <span className="text-muted-foreground">{userData.target || "₹0"} goal</span>
                            </div>
                            <div className="h-2 w-full bg-muted border border-border rounded-full overflow-hidden">
                                <div className="h-full bg-primary" style={{ width: userData.attainment || "0%" }} />
                            </div>
                            <p className="text-[10px] text-muted-foreground pt-1 flex items-center gap-1">
                                <TrendingUp className="h-2.5 w-2.5 text-success" /> {userData.attainment || "0%"} of quota met
                            </p>
                        </div>
                    </div>
                </div>
