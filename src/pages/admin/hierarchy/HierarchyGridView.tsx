import { User, Users, ChevronRight, Edit2, Trash2, Info, Plus } from "lucide-react";
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 py-2 animate-in fade-in duration-500">
      {currentChildren.map(node => (
        ...
        >
          ...
          <div className="px-5 py-3 bg-muted/20 border-t border-border flex items-center justify-between">
            <Button variant="ghost" size="sm" className="h-8 text-[11px] font-semibold text-primary hover:bg-primary/10 gap-1.5">
              <ChevronRight className="h-3.5 w-3.5" /> Explore
            </Button>
            <div className="flex items-center gap-1" onClick={e => e.stopPropagation()}>
              <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-muted-foreground hover:text-foreground"><Edit2 className="h-3.5 w-3.5" /></Button>
              <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-destructive hover:bg-destructive/10"><Trash2 className="h-3.5 w-3.5" /></Button>
            </div>
          </div>
        </div>
      ))}
      ...
      >
        ...
        <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Add Member</p>
      </button>
    </div>
  );
};
