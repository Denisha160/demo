...
import { useLeadInterestedProducts, useAddInterestedProduct, useRemoveInterestedProduct } from "@/hooks/useLeadInterestedProducts";
...
const ProductsTab = () => {
    ...
    const { data: interestedProducts = [], isLoading } = useLeadInterestedProducts(leadId);
    ...
    const [productToDelete, setProductToDelete] = useState<InterestedProduct | null>(null);
    ...
    const handleDelete = (productId: string) => {
        if (!leadId) return;
        removeProductMutation.mutate({ leadId, productId }, {
            onSuccess: () => setProductToDelete(null)
        });
    };

    const filteredData = interestedProducts.filter((item) =>
    ...
    const columns: Column<InterestedProduct>[] = [
        ...
        { 
            ...
            header: "Price",
            render: (item) => (
                <span className="font-mono font-bold text-primary text-xs">₹{Number(item.selling_price || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
            )
        },
        {
            key: "id",
            ...
            <div className="flex justify-between items-center mb-6 flex-wrap gap-4">
                <div className="flex items-center gap-3">
                    <div className="space-y-1">
                        <Label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Add Product</Label>
                        <Combobox
                ...
                <div className="space-y-1">
                    <Label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Search List</Label>
            ...
            <AlertDialog open={!!productToDelete} onOpenChange={(open) => !open && setProductToDelete(null)}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            This will remove "{productToDelete?.product_name}" from the lead's interested products.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
