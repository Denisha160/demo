    ...
    return (
        <div className="space-y-2 min-w-0">
            ...
            {isEditing ? (
                <div className="space-y-1">
                    {type === "textarea" ? (
                        ...
                        <Select value={value || ""} onValueChange={onChange}>
                            <SelectTrigger className={`h-8 text-xs rounded-sm ${error ? "border-destructive border" : ""}`}>
                ...
                type === "textarea" ? (
                    <p className="text-xs font-semibold leading-relaxed text-foreground/80">{value || "—"}</p>
                ) : (
                    <p className="text-sm font-semibold text-foreground truncate">{displayValue()}</p>
                )
            )}
        </div>
    );
