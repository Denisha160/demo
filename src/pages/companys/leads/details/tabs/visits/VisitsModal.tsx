...
const visitSchema = z.object({
  ...
  location_latitude: z
    .string()
    .min(1, "Location latitude is required (use Live Location)"),
  location_longitude: z
    .string()
    .min(1, "Location longitude is required (use Live Location)"),
...
interface VisitsModalProps {
  ...
  onSave: (
    data: VisitFormData,
    setError: UseFormSetError<VisitFormData>,
  ) => void;
...
const VisitsModal = ({
  ...
  useEffect(() => {
    if (contactData?.contacts) {
      setNameOptions(
        contactData.contacts.map((c) => ({ value: c.name, label: c.name })),
      );
      setDesignationOptions(
        Array.from(
          new Set(
            contactData.contacts.map((c) => c.designation).filter(Boolean),
          ),
        ).map((d) => ({ value: d!, label: d! })),
      );
      setPhoneOptions(
        Array.from(
          new Set(contactData.contacts.map((c) => c.phone).filter(Boolean)),
    ...
    if (visitData) {
      form.reset({
        ...
        location_longitude: visitData.location_longitude
          ? String(visitData.location_longitude)
          : "",
        ...
        customer_rating: visitData.customer_rating
          ? String(visitData.customer_rating)
          : "",
        ...
        scheduled_time: formatDateTimeLocal(
          visitData.scheduled_time || new Date(),
        ),
  ...
  const handleImageChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      reader.onloadend = () => {
        form.setValue("visit_image", String(reader.result || ""), {
          shouldDirty: true,
        });
        form.setValue("image_url", String(reader.result || ""), {
          shouldDirty: true,
        });
        form.setValue("visit_image_name", compressedFile.name, {
          shouldDirty: true,
        });
        form.setValue("visit_image_file", compressedFile, {
          shouldDirty: true,
        });
      };
    ...
    <Modal
      ...
      description={
        visitData
          ...
          : "Enter visit details for this lead."
      }
                      ...
                      <div className="overflow-hidden rounded-lg border border-border/60 bg-muted/20">
                        ...
                        <div className="flex items-center justify-between gap-3 border-t border-border/60 bg-background/90 p-3">
                          <span className="truncate text-xs text-muted-foreground">
                            {form.getValues("visit_image_name") ||
                              "Uploaded image"}
                          </span>
                          <Button
                            type="button"
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <ComboboxWithAdd
                      ...
                      value={field.value}
                      onValueChange={(val) =>
                        handleContactNameChange(val, field.onChange)
                      }
