...
export const useBrandList = (params?: Record<string, unknown>) => {
    return useQuery<BrandListResponse>({
        ...
        queryFn: async () => {
            const response = await listBrands(params) as ApiResponse<BrandListResponse>;
...
export const useBrandCombobox = (params?: Record<string, unknown>) => {
    return useQuery<Brand[]>({
        ...
        queryFn: async () => {
            const response = await listBrands({ ...params, combobox: true }) as ApiResponse<BrandComboboxResponse>;
...
export const useCreateBrand = () => {
    ...
    return useMutation<ApiResponse<unknown>, Error, BrandCreatePayload>({
        ...
        onError: (error) => {
            ...
            const message = apiError.response?.data?.message || apiError.message || 'Failed to create brand';
            toast.error(message);
        },
    });
};
...
export const useUpdateBrand = () => {
    ...
    return useMutation<ApiResponse<unknown>, Error, BrandUpdatePayload>({
        ...
        onError: (error) => {
            const apiError = error as ApiError;
            const message = apiError.response?.data?.message || apiError.message || 'Failed to update brand';
            toast.error(message);
        },
    });
};
...
export const useDeleteBrand = () => {
    ...
    return useMutation<ApiResponse, Error, string>({
        ...
        onError: (error) => {
            const apiError = error as ApiError;
            const message = apiError.response?.data?.message || apiError.message || 'Failed to delete brand';
            toast.error(message);
        },
    });
};
