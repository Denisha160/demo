        ...
        <div className="min-h-screen bg-gray-50 dark:bg-[#0a0a0a] py-12 px-4 sm:px-6 lg:px-8">
            <div className="max-w-6xl mx-auto">
                        ...
                        <div className="p-8 md:p-12 flex flex-col justify-center">
                            ...
                            <div className="grid grid-cols-2 gap-4 mb-10">
                                <div className="p-4 rounded-2xl bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/10">
                                    <div className="text-xs text-gray-500 uppercase font-bold mb-1">Brand</div>
                                    <div className="font-bold text-gray-900 dark:text-white">{product.brand}</div>
                                </div>
                                <div className="p-4 rounded-2xl bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/10">
                                    <div className="text-xs text-gray-500 uppercase font-bold mb-1">Stock</div>
                                    <div className="font-bold text-gray-900 dark:text-white">{product.stock > 0 ? `${product.stock} Units Available` : 'Out of Stock'}</div>
                                </div>
                            </div>

                            <button className="w-full py-5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-bold rounded-2xl transition-all shadow-lg shadow-blue-500/25 active:scale-[0.98]">
                                Add to Cart
                            </button>
                        </div>
                    </div>
                ...
                {product.images.length > 1 && (
                    <div className="mt-12">
                        <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">Product Gallery</h2>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            {product.images.slice(0, 4).map((img, idx) => (
                                <div key={idx} className="relative aspect-square rounded-2xl overflow-hidden border border-gray-200 dark:border-white/10 shadow-sm">
                                    <Image
                                        src={img}
