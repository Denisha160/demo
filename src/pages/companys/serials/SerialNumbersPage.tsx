...
import {
    Search, Hash, Plus, Printer, Download,
    RefreshCw, ShoppingCart,
    CheckCircle2, AlertCircle, Clock, Trash2, ChevronDown,
    X, Package, Box, Filter
} from "lucide-react";
...
const statusMap: Record<string, { label: string; variant: "success" | "secondary" | "destructive" | "warning" | "info" }> = {
...
const SerialNumbersPage = () => {
    ...
    const columns: Column<Serial>[] = [
            ...
            render: (item) => (
                <div className="flex items-center gap-2.5">
                    ...
                    <div className="min-w-0">
                        <p className="text-xs font-bold font-mono text-foreground">{item.serial_number}</p>
        ...
        },
        {
            ...
            header: "Product Code",
            render: (item) => (
                <Badge variant="secondary" className="text-[10px] font-mono px-1.5 h-5 uppercase">
                    {item.product_code}
                </Badge>
            )
        },
        ...
        },
        {
            ...
            header: "Status",
            render: (item) => {
                const config = statusMap[item.status] || { label: item.status, variant: "secondary" };
        ...
        <div className="w-full mx-auto space-y-4 animate-fade-in pb-10">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                ...
                <SummaryCard
                    ...
                    accent="bg-rose-500"
                    className={damagedCount > 0 ? "border-rose-200 bg-rose-50/30 dark:border-rose-900/40 dark:bg-rose-900/10" : ""}
                />
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 border-b border-border pb-2">
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto">
                    ...
                    <Button variant="outline" size="sm" className="h-8 text-xs rounded-sm gap-2">
            ...
            <div className="border border-border/60 rounded-sm shadow-sm overflow-hidden bg-card">
                <DataTable
                    ...
                    onServerPageSizeChange={(s) => { setLimit(s); setPage(1); }}
                />
            </div>
            ...
            <div className="flex items-center gap-2 p-2 bg-muted/20 rounded-sm border border-dashed border-border/60 text-[10px] text-muted-foreground">
                <span className="font-bold uppercase px-2 py-0.5 bg-background rounded border border-border">Bulk Actions:</span>
                <Button variant="ghost" className="h-6 text-[10px] gap-1 hover:bg-background">Change Status <ChevronDown className="h-3 w-3" /></Button>
                <Button variant="ghost" className="h-6 text-[10px] hover:bg-background">Print Labels</Button>
                <Button variant="ghost" className="h-6 text-[10px] hover:bg-background">Export</Button>
            </div>
        </div>
    );
};
