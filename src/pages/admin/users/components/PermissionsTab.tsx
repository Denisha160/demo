...
const PermissionsTab = ({
  ...
  const dynamicCompanies = companiesList.map(
    (c: Company) => c.display_name || c.legal_name,
  );
  ...
  const isDirty =
    JSON.stringify(companyRoles) !== JSON.stringify(initialCompanyRoles);
  ...
  const handleSave = () => {
    ...
    for (const [companyName, roleName] of Object.entries(companyRoles)) {
      const company = companiesList.find(
        (c: Company) => (c.display_name || c.legal_name) === companyName,
      );
                      ...
                      <div className="flex items-center gap-3">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) =>
                            handleToggleCompany(company, checked === true)
                          }
                          onClick={(e) => e.stopPropagation()} // Prevent triggering the outer div click
                        />
                        <span
                          className={`font-bold text-sm ${isActiveTab ? "text-primary" : "text-foreground"}`}
                        >
                          {company}
                        </span>
                      </div>
                        ...
                        {!isCompanyEnabled ? (
                          ...
                          rolesList.map((role) => {
                            const isRoleSelected =
                              companyRoles[company] === role.name;
                            return (
                              <div
                                key={role.id}
                                onClick={() =>
                                  handleRoleSelect(company, role.name)
                                }
                              ...
                              >
                                <div className="flex-1 min-w-0">
                                  <p
                                    className={`text-xs font-bold leading-none ${isRoleSelected ? "text-primary-foreground" : "text-foreground"}`}
                                  >
                                    {role.name}
                                  </p>
                                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              {isLoadingPermissions ? (
                ...
                groups.map((group) => (
                  <div
                    key={group.module}
                    className="border border-border rounded-sm bg-muted/10 overflow-hidden"
                  >
                    ...
                    <div className="p-3 space-y-2">
                      {group.permissions.map((permission) => (
                        <div
                          key={permission.id}
                          className="flex items-center gap-3 p-1.5 rounded-sm bg-background border border-border"
                        >
                          {permission.enabled ? (
                          ...
                          <span
                            className={`text-xs ${permission.enabled ? "font-semibold text-foreground" : "text-muted-foreground"}`}
                          >
                            {permission.label}
                          </span>
                        </div>
