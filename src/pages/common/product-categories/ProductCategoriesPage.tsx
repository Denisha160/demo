...
const ProductCategoriesPage = () => {
    ...
    setSearchParams(
      (prev) => {
        ...
        if (mainCategoryIdFilter)
          next.set("mainCategory", mainCategoryIdFilter);
  ...
  const { data: mainCategories = [], isLoading: isLoadingCombobox } =
  ...
  const categories: ProductCategory[] = (listResponse?.items || []).map(
    toDisplay,
  );
          ...
          {(filterType === "All" || filterType === "sub") && (
            <div className="w-[200px] animate-in fade-in slide-in-from-left-2 duration-300">
              <Combobox
                ...
                placeholder={
                  isLoadingCombobox ? "Loading..." : "Filter by Main Category"
                }
