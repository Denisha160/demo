...
const generateThemeFromId = (
    ...
    const saturationRange = config.saturationRange || DEFAULT_THEME.saturationRange!;
    ...
    const saturation = saturationRange[0] + (hash % (saturationRange[1] - saturationRange[0]));
    const lightness = lightnessRange[0] + ((hash >> 8) % (lightnessRange[1] - lightnessRange[0]));

    return {
        primary: `${baseHue} ${saturation}% ${lightness}%`,
        secondary: config.secondary || `${secondaryHue} ${saturation - 10}% ${lightness - 8}%`,
        accent: config.accent || `${accentHue} ${saturation - 15}% ${lightness + 5}%`,
...
export const getThemeCSSVariables = (theme: CompanyTheme): Record<string, string> => {
    return {
        ...
        "--border": theme.customColors?.border || `${theme.primary.split(" ")[0]} 32.6% 17.5%`,
        "--input": theme.customColors?.input || `${theme.primary.split(" ")[0]} 32.6% 17.5%`,
        "--muted": theme.customColors?.muted || `${theme.primary.split(" ")[0]} 32.6% 17.5%`,
