...
const PackagesPage = () => {
    ...
    const hasFilters = Boolean(search || filterType !== "All" || sortKey !== "created_at");
    ...
    const columns: Column<PackageType>[] = [
            ...
            render: (item) => (
                ...
                >
                    ...
                    <div className="min-w-0">
                        <p className="text-sm font-semibold text-foreground truncate">{item.package_name}</p>
                        <p className="text-[10px] text-muted-foreground uppercase tracking-wider truncate">{item.package_code}</p>
                    </div>
                </div>
            )
        },
        {
            ...
            header: "Type",
            sortable: true,
            render: (item) => <StatusBadge status={item.package_type || "N/A"} variant="info" />
        },
        {
            key: "dimensions",
        ...
        },
        {
            ...
            render: (item) => (
                <div className="flex flex-col">
                    <span className="text-sm font-semibold">{Number(item.volume_cubic_cm).toLocaleString()} cm³</span>
                    <span className="text-[10px] text-muted-foreground">CBM: {Number(item.cbm).toFixed(6)}</span>
                </div>
            )
        },
        {
    ...
    return (
        <div className="w-full mx-auto space-y-2 animate-fade-in">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 border-b border-border pb-4">
                ...
                <Button size="sm" className="h-8 text-xs rounded-sm gap-2 flex-1 sm:flex-none" onClick={handleCreate}>
            ...
            <AlertDialog open={!!packageToDelete} onOpenChange={(open) => !open && setPackageToDelete(null)}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            This will permanently delete the package "{packageToDelete?.package_name}".
