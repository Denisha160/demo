...
const ProductInput = ({
    ...
    <div className={`space-y-1.5 ${className}`}>
        ...
        <div className={prefix ? "relative" : ""}>
            {prefix && <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-muted-foreground">{prefix}</span>}
            <Input
                type={type}
        ...
        {error && <p className="text-[10px] text-destructive mt-0.5 ml-0.5">{error}</p>}
    </div>
);
...
const KitFormPage = () => {
    ...
    const updateKitPrice = (currentItems: KitFormData["items"]) => {
        const newTotal = currentItems.reduce((sum, item) => sum + ((item.price || 0) * (item.quantity_per_kit || 0)), 0);
        setValue("kit_price", parseFloat(newTotal.toFixed(2)), { shouldDirty: true });
    };
    ...
    const handleAddProduct = (productId: string) => {
        ...
        const existingIdx = items.findIndex(item => item.finished_product_id === productId);
        if (existingIdx > -1) {
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-2">
                ...
                <div className="flex items-center gap-2 flex-wrap justify-end">
                    ...
                    >
                        ...
                        {isCreating || isUpdating ? (isEditing ? "Updating..." : "Saving...") : (isEditing ? "Update Kit" : "Save Kit")}
                    </Button>
                </div>
            </div>
                ...
                <div className="lg:col-span-1 space-y-4">
                    <div className="bg-card border border-border rounded-lg p-5 shadow-sm space-y-5">
                        <div className="flex items-center gap-2 mb-1">
                            ...
                            <h3 className="text-[10px] font-bold text-foreground uppercase tracking-widest">Kit Information</h3>
                        </div>
                        ...
                        <div className="grid grid-cols-1 gap-4">
                            ...
                            <div className="space-y-1.5">
                                ...
                                <Combobox
                                    ...
                                    onValueChange={(val) => setValue("packaging_id", val, { shouldDirty: true })}
                            ...
                            <div className="pt-1 px-1">
                                <Label className="text-[9px] font-bold text-muted-foreground uppercase opacity-70">
                                    Market Sum Value: <span className="text-foreground ml-1">₹{totalValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                                </Label>
                            </div>
                        </div>
                        ...
                        <div className="p-5 border-b border-border bg-muted/20">
                            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                                <div className="flex items-center gap-2">
                                    ...
                                    <h3 className="text-[10px] font-bold text-foreground uppercase tracking-widest">Included Products</h3>
                                </div>
                            ...
                            <table className="w-full text-sm">
                                <thead className="bg-muted/30 text-[10px] uppercase tracking-wider font-bold text-muted-foreground sticky top-0 bg-background border-b border-border z-10">
                                    <tr>
                                        ...
                                        <th className="px-5 py-3 text-center w-[120px]">Quantity</th>
                                        <th className="px-5 py-3 text-center w-[100px]">Unit</th>
                                        <th className="px-5 py-3 text-right w-[150px]">Est. Value</th>
                                        ...
                                        <tr>
                                            <td colSpan={5} className="py-20 text-center text-muted-foreground italic">
                                                <div className="flex flex-col items-center gap-2 opacity-50">
                                                    ...
                                                    <p className="text-xs">No products selected yet. Use the search bar above to add items.</p>
                                                </div>
                                            </td>
                                        </tr>
                                    ) : (
                                            ...
                                            <tr key={index} className="hover:bg-muted/10 transition-colors group">
                                                <td className="px-5 py-3">
                                                    <div className="flex items-center gap-3">
                                                        {item.image_url ? (
                                                            <img src={item.image_url} alt={item.product_name} className="w-10 h-10 object-cover rounded-md border border-border" />
                                                        ) : (
                                                        ...
                                                        <div className="flex flex-col">
                                                            ...
                                                            <span className="text-[10px] text-muted-foreground font-mono">#{item.finished_product_id.split('-')[0].toUpperCase()}</span>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="px-5 py-3">
                                                ...
                                                <td className="px-5 py-3 text-center">
                                                    <Badge variant="secondary" className="text-[9px] font-bold rounded-sm h-5 px-1.5 uppercase bg-muted text-muted-foreground border-border">
                                                        {item.unit || "pcs"}
                                                    </Badge>
                                                </td>
                                                <td className="px-5 py-3 text-right">
                                                    <div className="flex flex-col">
                                                        <span className="font-mono font-bold text-primary text-xs">
                                                            ₹{((item.price || 0) * (item.quantity_per_kit || 0)).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                                        </span>
                                                        <span className="text-[9px] text-muted-foreground italic">₹{(item.price || 0).toFixed(2)} / unit</span>
                                                    </div>
                                                </td>
                                                <td className="px-5 py-3 text-center">
                                                    <Button
                        ...
                        {items.length > 0 && (
                            <div className="p-5 bg-muted/10 border-t border-border flex flex-col sm:flex-row items-center justify-between gap-4">
                                <div className="text-[11px] text-muted-foreground font-medium">
                                    Total Items: <span className="text-foreground font-bold">{items.reduce((sum, i) => sum + (Number(i.quantity_per_kit) || 0), 0)} Units</span> Across <span className="text-foreground font-bold">{items.length} Products</span>
                                </div>
                                <div className="flex items-center gap-3">
                                    <span className="text-[10px] uppercase tracking-widest font-bold text-muted-foreground">Kit Selling Price</span>
                                    <div className="relative">
                                        ...
                                        <Input
                                            ...
                                            onChange={(e) => setValue("kit_price", parseFloat(e.target.value) || 0, { shouldDirty: true })}
