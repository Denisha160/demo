...
const ImportPage = () => {
    ...
    const statusOptions = (statusResponse as any)?.items?.map((item: any) => ({
        value: item.id,
        label: item.name,
    })) || [];
            ...
            <div className="flex items-center justify-between border-b border-border bg-card px-6 py-4 shadow-sm">
                <div className="flex items-center gap-4">
                    ...
                    <div className="space-y-0.5">
                        <h1 className="text-xl font-bold tracking-tight text-foreground">Import Leads</h1>
                        <p className="text-xs text-muted-foreground">Upload and manage your lead data from CSV files</p>
                    </div>
                </div>
                <Button
                    variant="outline"
                ...
                <div className="mx-auto max-w-6xl space-y-8">
                        ...
                        <div className="rounded-xl border border-blue-500/10 bg-blue-500/5 p-6 backdrop-blur-sm">
                            ...
                            <ul className="grid gap-4 md:grid-cols-1">
                                <li className="flex gap-3 text-sm leading-relaxed text-muted-foreground">
                                    <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-blue-500/10 text-[10px] font-bold text-blue-600">1</span>
                                    <p>Your CSV data should be in the format below. The first line of your CSV file should be the <strong>column headers</strong> as in the table example. Also make sure that your file is <strong>UTF-8</strong> to avoid unnecessary <strong>encoding problems</strong>.</p>
                                </li>
                                <li className="flex gap-3 text-sm leading-relaxed text-muted-foreground">
                                    <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-blue-500/10 text-[10px] font-bold text-blue-600">2</span>
                                    <p>If the column <strong>you are trying to import is date</strong> make sure that is formatted in <strong>format d-m-yyyy (27-03-2026)</strong>.</p>
                                </li>
                                <li className="flex gap-3 text-sm leading-relaxed text-muted-foreground">
                                    <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-blue-500/10 text-[10px] font-bold text-blue-600">3</span>
                                    <div className="space-y-2">
                                        <p>Based on your leads <strong>unique validation</strong> configured <strong>options</strong>, the lead <strong>won't be imported</strong> if:</p>
                                        ...
                                        <p className="text-[11px] italic">If you still want to import all leads, uncheck all unique validation fields in settings.</p>
                                    </div>
                                </li>
                            </ul>
                        </div>
                    ...
                    <section className="space-y-3 animate-in fade-in slide-in-from-top-6 duration-700">
                        <div className="flex items-center justify-between">
                            <h2 className="text-sm font-bold text-foreground">Header Example</h2>
                            <span className="text-[10px] font-medium text-muted-foreground bg-muted px-2 py-0.5 rounded-full">Preview Only</span>
                        </div>
                    ...
                    <section className="grid gap-6 md:grid-cols-2 animate-in fade-in slide-in-from-bottom-6 duration-700">
                        <div className="space-y-4 rounded-xl border border-border/60 bg-card p-6 shadow-sm">
                            ...
                            <div className="space-y-2">
                                <Label htmlFor="csv-file" className="text-xs font-bold text-muted-foreground flex items-center gap-1">
