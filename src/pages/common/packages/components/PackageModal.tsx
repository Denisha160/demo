...
import { PackageCreatePayload, PackageUpdatePayload, ApiErrorResponse } from "@/types/packages";
import { Loader2 } from "lucide-react";

// Validation schema
const packageSchema = z.object({
    package_code: z.string().min(3, "Package code must be at least 3 characters").max(20, "Package code too long"),
    ...
    length_cm: z.string().refine((val) => !isNaN(parseFloat(val)) && parseFloat(val) > 0, "Length must be a positive number"),
    width_cm: z.string().refine((val) => !isNaN(parseFloat(val)) && parseFloat(val) > 0, "Width must be a positive number"),
    height_cm: z.string().refine((val) => !isNaN(parseFloat(val)) && parseFloat(val) > 0, "Height must be a positive number"),
...
const PackageModal = ({ isOpen, onClose, packageId }: PackageModalProps) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const handleError = (error: unknown) => {
            ...
            if (errorData?.code === "validation_error") {
                const bodyErrors = errorData.details?.body || errorData.details?.params || {};
                setErrors(bodyErrors);
            } else if (errorData?.code === "duplicate_key_value") {
                const msg = errorData.message || "A package with this code already exists.";
    ...
    return (
        <Modal
            ...
            description={isEditing ? "Modify package dimensions and details" : "Fill in the required information below to create a new package type"}
            footer={
                <>
                    <Button
                        variant="outline"
                ...
                <form onSubmit={handleSubmit} className="space-y-4 mt-2">
                    <div className="grid grid-cols-2 gap-3">
                        <div className="space-y-1">
                            ...
                            />
                            {errors.package_code && (
                                <p className="text-[10px] text-destructive mt-1 font-medium">{errors.package_code}</p>
                            )}
                        </div>
                        <div className="space-y-1">
                            ...
                            <Select
                                ...
                                disabled={isPending}
                            >
                                <SelectTrigger className={`h-8 text-sm rounded-sm ${errors.package_type ? 'border-destructive' : ''}`}>
                            ...
                            {errors.package_type && (
                                <p className="text-[10px] text-destructive mt-1 font-medium">{errors.package_type}</p>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="space-y-1">
                        ...
                        />
                        {errors.package_name && (
                            <p className="text-[10px] text-destructive mt-1 font-medium">{errors.package_name}</p>
                        )}
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                        <div className="space-y-1">
                            ...
                            />
                            {errors.length_cm && (
                                <p className="text-[10px] text-destructive mt-1 font-medium">{errors.length_cm}</p>
                            )}
                        </div>
                        <div className="space-y-1">
                            ...
                            />
                            {errors.width_cm && (
                                <p className="text-[10px] text-destructive mt-1 font-medium">{errors.width_cm}</p>
                            )}
                        </div>
                        <div className="space-y-1">
                            ...
                            />
                            {errors.height_cm && (
                                <p className="text-[10px] text-destructive mt-1 font-medium">{errors.height_cm}</p>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="p-3 bg-primary/5 border border-primary/10 rounded-sm flex justify-between items-center text-xs">
                        <div className="flex flex-col gap-0.5">
                            <span className="text-muted-foreground uppercase tracking-wider font-semibold text-[9px]">Total Volume</span>
                            <span className="font-bold text-primary">{volume.toLocaleString()} cm³</span>
                        </div>
                        <div className="flex flex-col items-end gap-0.5">
                            <span className="text-muted-foreground uppercase tracking-wider font-semibold text-[9px]">CBM Index</span>
                            <span className="font-bold text-primary">{cbm.toFixed(6)} m³</span>
                        </div>
                    </div>
                </form>
            </div>
