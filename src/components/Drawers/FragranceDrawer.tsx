...
export function FragranceDrawer({ open, onOpenChange }: FragranceDrawerProps) {
  ...
  const { mutate: createFragrance, isPending: isCreating } =
    useCreateFragrance();
  const { mutate: updateFragrance, isPending: isUpdating } =
    useUpdateFragrance();
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="space-y-1">
            ...
            <DrawerDescription className="text-sm text-muted-foreground">
              {formData.id
                ? "Modify existing fragrance details."
                : "Fill in the details for the new fragrance."}
            </DrawerDescription>
          </div>
        </div>
      </DrawerHeader>
