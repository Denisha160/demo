...
const LeadTable = ({ displayedColumns }: LeadTableProps) => {
    ...
    const tableColumns: Column<Deal & { stage: string; stageVariant: string }>[] = [
        { 
            ...
            render: (item) => (
                <div className="flex flex-col gap-0.5">
                    <span className="font-semibold text-foreground text-sm uppercase">{item.title}</span>
                </div>
            )
        },
        { 
            ...
            render: (item) => <span className="text-sm text-foreground/80">{item.company || "-"}</span>
        },
        { 
            key: "contact", 
                    ...
                    <DropdownMenuContent align="end" className="w-56 p-1">
                        {leadStatuses.map((status) => (
                            ...
                            >
                                ...
                                {item.status_id === status.id && <Check className="h-3.5 w-3.5 text-primary" />}
                            </DropdownMenuItem>
                        ))}
                    </DropdownMenuContent>
                </DropdownMenu>
                ...
                <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                        ...
                        >
                            <span className={cn(
                                ...
                                PRIORITIES.find(p => p.value === item.priority)?.color || "text-muted-foreground"
                            )}>
                    ...
                    <DropdownMenuContent align="end" className="w-36 p-1">
                        {PRIORITIES.map((p) => (
                            ...
                            >
                                <span className={cn("font-extrabold uppercase tracking-wide", p.color)}>
                                    {p.label}
                                </span>
                                {item.priority === p.value && <Check className="h-3.5 w-3.5 text-primary" />}
                            </DropdownMenuItem>
                        ))}
                    </DropdownMenuContent>
                </DropdownMenu>
        ...
        { 
            ...
            render: (item) => <span className="text-[10px] text-muted-foreground font-medium">{item.date}</span>
        },
    ];
