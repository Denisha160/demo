    ...
    <Modal
      ...
      footer={
        <>
          ...
          <Button
            size="sm"
            className="rounded-sm text-xs h-8 bg-primary"
        ...
        <div className="p-3 bg-muted/30 rounded-sm border border-border/50">
          ...
          <p className="text-lg font-bold text-foreground">
            ...
            {formData.amount && formData.tenure
              ? (
                  Number(formData.amount) / Number(formData.tenure)
                ).toLocaleString(undefined, { maximumFractionDigits: 0 })
              : "0"}
            <span className="text-[10px] font-normal text-muted-foreground lowercase ml-1">
              / month
            </span>
          </p>
        </div>
