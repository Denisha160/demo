...
const ProductsPage = () => {
  ...
  useEffect(() => {
    setSearchParams(
      (prev) => {
        ...
        if (categoryIdFilter) next.set("categoryId", categoryIdFilter);
        else next.delete("categoryId");
        if (statusFilter !== "all") next.set("status", statusFilter);
        else next.delete("status");
        if (brandIdFilter) next.set("brandId", brandIdFilter);
  ...
  const columns: Column<Product>[] = [
    ...
    },
    {
      ...
      render: (item) => (
        <StatusBadge
          status={item.product_type.replace("_", " ")}
          variant="info"
        />
      ),
    },
    ...
    },
    {
      ...
      render: (item) => (
        <div className="flex flex-col">
          <span
            className={`font-semibold ${Number(item.stock) <= 0 ? "text-destructive" : ""}`}
          >
