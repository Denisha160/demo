import { FlaskConical, Loader2, Edit, Calendar, MapPin, Package, Box } from "lucide-react";
...
type StatusVariant = "success" | "secondary" | "destructive" | "warning" | "info";
...
const getExpiryInfo = (expiryDate: string | null) => {
    if (!expiryDate) return { label: "No expiry set", className: "text-muted-foreground" };
    const days = differenceInDays(new Date(expiryDate), new Date());
    if (isPast(new Date(expiryDate))) return { label: "Expired", className: "text-destructive font-bold" };
    if (days <= 30) return { label: `Expires in ${days} days ⚠️`, className: "text-amber-500 font-bold" };
    return { label: format(new Date(expiryDate), "dd MMM yyyy"), className: "text-foreground" };
};
...
const BatchViewModal = ({ open, onClose, batchId, onEdit }: BatchViewModalProps) => {
    ...
    return (
        <Modal
            ...
            title={isLoading ? "Loading..." : (batch?.batch_number ?? "Batch Details")}
            description={batch ? `${batch.product_name} · ${batch.product_code}` : "View batch details"}
            footer={
                <>
                    ...
                    {batch && onEdit && (
                        <Button
                            ...
                            onClick={() => { onClose(); onEdit(batch.id); }}
                        >
            ...
            {isLoading ? (
                ...
                <div className="space-y-5 mt-2">
                    ...
                    <div className="flex items-center gap-3 p-3 bg-muted/30 rounded-sm border border-border/60">
                        ...
                        <div>
                            <p className="text-sm font-bold text-foreground">{batch.product_name}</p>
                            <p className="text-[10px] font-mono text-muted-foreground uppercase tracking-wider">{batch.product_code}</p>
                        </div>
                    ...
                    <div className="space-y-2">
                        <div className="flex items-center justify-between text-xs">
                            ...
                            <span className="font-bold text-foreground">
                                {Number(batch.remaining_quantity).toLocaleString()} / {Number(batch.initial_quantity).toLocaleString()}
                            </span>
                        </div>
                        <div className="h-2.5 w-full bg-muted rounded-full overflow-hidden">
                            {(() => {
                                const pct = getStockPercent(Number(batch.remaining_quantity), Number(batch.initial_quantity));
                                return (
                                    <div
                        ...
                        <p className="text-[10px] text-muted-foreground">
                            {getStockPercent(Number(batch.remaining_quantity), Number(batch.initial_quantity))}% remaining
                        </p>
                    </div>
                        ...
                        {[
                            {
                                ...
                                value: format(new Date(batch.manufacturing_date), "dd MMM yyyy"),
                            },
                            {
                                label: "Expiry Date",
                                icon: <Calendar className="h-3 w-3" />,
                            ...
                            },
                            {
                                ...
                                value: batch.location || <span className="italic text-muted-foreground/50">Not specified</span>,
                            },
                            {
                                label: "Unit",
                            ...
                            <div key={row.label} className="bg-muted/20 rounded-sm p-3 border border-border/50 space-y-1">
                    ...
                    {batch.notes && (
                        <div className="rounded-sm border border-border/60 bg-muted/20 p-3">
                            <p className="text-[10px] text-muted-foreground font-semibold uppercase tracking-wide mb-1">Notes</p>
