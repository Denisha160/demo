...
import { useKitsByProduct, useKitList, useAssociateProductToKit, useDisassociateProductFromKit } from "@/hooks/useKits";
...
export const KitsTab = ({ productId, productType, isNew }: KitsTabProps) => {
  ...
  const columns: Column<KitMembership>[] = [
    ...
    },
    {
      ...
      header: "Qty / Kit",
      className: "w-[120px]",
      render: (item) => <span className="text-xs font-bold font-mono text-primary">{item.quantity_per_kit}</span>,
    },
    {
      key: "id",
      header: "",
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="space-y-3">
        ...
        <div className="rounded-sm border border-border/60 overflow-hidden shadow-inner bg-muted/5">
          <DataTable data={kitsByProduct} columns={columns} isLoading={isLoadingAssoc} pageSize={5} onRowClick={(item) => setViewKitId(item.id)} />
        </div>
      </div>

      <KitViewModal open={!!viewKitId} onClose={() => setViewKitId(undefined)} kitId={viewKitId} />
    </div>
  );
};
