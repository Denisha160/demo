...
const updateObjectState = <T extends Record<string, unknown>>(
  setter: React.Dispatch<React.SetStateAction<T[]>>,
  index: number,
  field: keyof T,
  val: T[keyof T],
...
const removeObjectState = <T extends Record<string, unknown>>(
  setter: React.Dispatch<React.SetStateAction<T[]>>,
  index: number,
) => {
  setter((prev) =>
...
const productSchema = z
  .object({
    ...
    category_id: z.preprocess(
      (val) => (val === "" ? null : val),
      z.string().uuid("Please select a valid category").optional().nullable(),
    ),
    ...
    max_stock_level: z
      .number()
      .min(0, "Max stock must be >= 0")
      .nullable()
      .optional(),
    reorder_point: z
      .number()
      .min(0, "Reorder point must be >= 0")
      .nullable()
      .optional(),

    shelf_life_days: z
      .number()
      .min(0, "Shelf life must be >= 0")
      .nullable()
      .optional(),
  ...
  .refine(
    (data) => {
      if (data.unit_category === "weight")
        return ["kg", "g"].includes(data.base_unit);
      if (data.unit_category === "volume")
        return ["ltr", "ml"].includes(data.base_unit);
...
interface ProductTabProps {
  ...
  metadata: {
    metaColors: string[];
    setMetaColors: (val: string[]) => void;
    metaFeatures: string[];
    setMetaFeatures: (val: string[]) => void;
    metaParams: { key: string; value: string }[];
    setMetaParams: (val: { key: string; value: string }[]) => void;
    metaAttrs: { key: string; value: string }[];
    setMetaAttrs: (val: { key: string; value: string }[]) => void;
  };
  drawers?: {
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        ...
        <div className="flex items-center justify-between">
          ...
          {adminFeature?.isRoot && (
            <Button
              ...
              onClick={() =>
                adminFeature.setShowSecret(!adminFeature.showSecret)
              }
            >
      ...
      <div className="space-y-2">
        ...
        >
          ...
          <SelectContent>
            <SelectItem value="FINISHED_GOOD" className="text-xs">
              Finished Good
            </SelectItem>
            <SelectItem value="SEMI_FINISHED" className="text-xs">
              Semi Finished
            </SelectItem>
            <SelectItem value="RAW_MATERIAL" className="text-xs">
              Raw Material
            </SelectItem>
          </SelectContent>
        </Select>
      ...
      <div className="space-y-2">
        ...
        <Combobox
          ...
          clearable
          searchValue={comboboxes.category.search}
          onSearchChange={comboboxes.category.setSearch}
        />
        <Button
          type="button"
          variant="ghost"
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="space-y-2">
          ...
          <Combobox
            ...
            value={productData.fragrance_id ?? ""}
            onValueChange={(v) => handleChange("fragrance_id", v)}
            placeholder="Select fragrance..."
            searchPlaceholder="Search fragrance..."
            emptyText="No fragrances found."
            className={`h-9 ${errors.fragrance_id ? "border-destructive" : ""}`}
            clearable
            searchValue={comboboxes.fragrance.search}
            onSearchChange={comboboxes.fragrance.setSearch}
          />
          <Button
            type="button"
            variant="ghost"
        ...
        {productData.is_brand && (
          <div className="space-y-2">
            ...
            <Combobox
              ...
              clearable
              searchValue={comboboxes.brand.search}
              onSearchChange={comboboxes.brand.setSearch}
            />
            <Button
              type="button"
              variant="ghost"
      ...
      <div className="space-y-2 lg:col-span-1">
        ...
        <Select
          ...
          onValueChange={(v) => {
            ...
            setProductData((prev) => ({
              ...prev,
              base_unit: v as "kg" | "g" | "ltr" | "ml" | "pcs",
        ...
        >
          <SelectTrigger
            className={`text-sm ${errors.base_unit ? "input-error border-destructive" : ""}`}
          >
        ...
        <div className="bg-muted/30 p-4 rounded-md border flex flex-col gap-3">
          <div className="flex justify-between items-center w-full">
            ...
            <Button
              ...
              variant="outline"
              size="sm"
              onClick={() => setMetaColors([...metaColors, ""])}
            ...
            {metaColors.map((color, idx) => (
              ...
              >
                <Input
                  value={color}
                  onChange={(e) =>
                    updateArrayState(setMetaColors, idx, e.target.value)
                  }
        ...
        <div className="bg-muted/30 p-4 rounded-md border flex flex-col gap-3">
          <div className="flex justify-between items-center w-full">
            ...
            <Button
              ...
              variant="outline"
              size="sm"
              onClick={() => setMetaFeatures([...metaFeatures, ""])}
              className="h-6 px-2 text-[10px] uppercase tracking-wider"
            >
            ...
            {metaFeatures.map((feature, idx) => (
              ...
              >
                <Input
                  value={feature}
                  onChange={(e) =>
                    updateArrayState(setMetaFeatures, idx, e.target.value)
                  }
        ...
        <div className="bg-muted/30 p-4 rounded-md border flex flex-col gap-3">
          <div className="flex justify-between items-center w-full">
            ...
            <Button
              ...
              size="sm"
              onClick={() =>
                setMetaParams([...metaParams, { key: "", value: "" }])
              }
              className="h-6 px-2 text-[10px] uppercase tracking-wider"
            >
            ...
            {metaParams.map((param, idx) => (
              <div key={idx} className="flex gap-2 items-center w-full">
                <Input
                  value={param.key}
                  onChange={(e) =>
                    updateObjectState(setMetaParams, idx, "key", e.target.value)
                  }
        ...
        <div className="bg-muted/30 p-4 rounded-md border flex flex-col gap-3">
          <div className="flex justify-between items-center w-full">
            ...
            <Button
              ...
              size="sm"
              onClick={() =>
                setMetaAttrs([...metaAttrs, { key: "", value: "" }])
              }
              className="h-6 px-2 text-[10px] uppercase tracking-wider"
            >
            ...
            {metaAttrs.map((attr, idx) => (
              <div key={idx} className="flex gap-2 items-center w-full">
                <Input
                  value={attr.key}
                  onChange={(e) =>
                    updateObjectState(setMetaAttrs, idx, "key", e.target.value)
                  }
...
const RecipesTab = ({
  ...
  if (isNew) {
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center bg-muted/10 rounded-lg border border-dashed border-border transition-all hover:bg-muted/20">
        ...
        <p className="text-[11px] text-muted-foreground mt-1 max-w-xs leading-relaxed">
          Recipes can only be defined once the product record is created. Please
          save your changes and return to this tab.
        </p>
      </div>
    );
  ...
  if (productType === "RAW_MATERIAL") {
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center bg-muted/10 rounded-lg border border-dashed border-border">
        ...
        <p className="text-[11px] text-muted-foreground mt-1 max-w-xs leading-relaxed">
          Recipes are only applicable to{" "}
          <span className="text-primary font-bold">Finished Goods</span> and{" "}
          <span className="text-primary font-bold">Semi Finished Items</span>.
  ...
  const totalCost =
    bom?.raw_materials?.reduce(
      (sum, m) => sum + m.raw_quantity * (m.cost_price || 0),
      0,
    ) || 0;
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      {isLoading ? (
        ...
        >
          ...
          <p className="text-[11px] text-muted-foreground mt-1 max-w-xs leading-relaxed">
            This finished good doesn't have a Bill of Materials yet. Click to
            define the raw materials required for production.
          </p>
        </div>
      ) : (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            >
              ...
              <p
                className={`text-xl font-bold font-mono ${profit >= 0 ? "text-emerald-600" : "text-destructive"}`}
              >
                {profit >= 0 ? "+" : ""}₹
              ...
              <table className="w-full text-xs">
                <thead className="bg-muted/40 text-[10px] uppercase font-bold text-muted-foreground border-b border-border/60">
                  <tr>
                    ...
                    <th className="px-4 py-2.5 text-center w-[120px]">
                      Quantity
                    </th>
                    <th className="px-4 py-2.5 text-right w-[150px]">
                      Cost Impact
                    </th>
                  </tr>
                </thead>
                  ...
                  {bom.raw_materials.map((m) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-right font-mono font-bold text-primary/80">
                        ₹
                        {(m.raw_quantity * (m.cost_price || 0)).toLocaleString(
                          undefined,
                          { minimumFractionDigits: 2 },
                        )}
                      </td>
...
const ProductFormPage = () => {
  ...
  const [metaColors, setMetaColors] = useState<string[]>(
    () => getInitialMetadata({}).colors,
  );
  const [metaFeatures, setMetaFeatures] = useState<string[]>(
    () => getInitialMetadata({}).features,
  );
  const [metaParams, setMetaParams] = useState<
    { key: string; value: string }[]
  >(() => getInitialMetadata({}).params);
  const [metaAttrs, setMetaAttrs] = useState<{ key: string; value: string }[]>(
    () => getInitialMetadata({}).attrs,
  );
  ...
  const handleReset = useCallback(() => {
    ...
    if (fetchedProduct && !isNew) {
      ...
      setMetaColors([""]);
      setMetaFeatures([""]);
      setMetaParams([{ key: "", value: "" }]);
      setMetaAttrs([{ key: "", value: "" }]);
      setImagePreviews([]);
    ...
    try {
      const validData = productSchema.parse({
        ...
        size_value: productData.size_value
          ? Number(productData.size_value)
          : null,
        dimension_unit: productData.dimension_unit || null,
        brand_id: productData.brand_id || null,
        fragrance_id: productData.fragrance_id || null,
      ...
      if (isNew) {
        ...
        updateProduct(
          { ...payload, id: productData.id as string },
          {
            onError: onErrorHandler,
          },
        );
  ...
  const tabProps: ProductTabProps = {
    ...
    drawers: {
      category: {
        ...
        setOpen: setIsCategoryDrawerOpen,
      },
      fragrance: {
        open: isFragranceDrawerOpen,
        setOpen: setIsFragranceDrawerOpen,
      },
        ...
        <div className="p-4 border border-border rounded-lg bg-card shadow-sm space-y-6 animate-in fade-in-50 duration-300">
          ...
          >
            <TabsList className="flex flex-wrap h-auto w-full justify-start bg-transparent border-b border-border rounded-none pb-0 mb-6 gap-2">
              <TabsTrigger
                value="measurements"
                className="data-[state=active]:bg-muted data-[state=active]:border-b-primary rounded-t-md border-b-2 border-transparent px-4 py-2 text-xs uppercase tracking-wider font-semibold"
              >
                Units & Measurements
                {errors.base_unit || errors.weight ? (
            ...
            <div className="min-h-[300px]">
              <TabsContent value="measurements" className="m-0">
                <UnitsMeasurementsTab {...tabProps} />
              </TabsContent>
              <TabsContent value="packaging" className="m-0">
                <PackagingTab {...tabProps} />
              </TabsContent>
              <TabsContent value="pricing" className="m-0">
                <PricingTab {...tabProps} />
              </TabsContent>
              <TabsContent value="specifications" className="m-0">
                <SpecificationsTab {...tabProps} />
              </TabsContent>
            ...
            <div className="text-xs font-medium">
              {Object.keys(errors).length > 0 && (
                <span className="text-destructive flex items-center gap-1">
                  ...
                  {Object.keys(errors).length} field(s) need attention across
                  tabs
                </span>
              )}
            </div>
