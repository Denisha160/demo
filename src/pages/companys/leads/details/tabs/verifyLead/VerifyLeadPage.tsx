    ...
    <div className="space-y-2 animate-fade-in pb-10 w-full rounded-2xl border border-border/50 bg-card p-4 shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <InfoCard
          icon={Layout}
          label="Property Type"
          value={details.property_type?.replace(/_/g, " ")}
        ...
        <InfoCard
          ...
          value={
            details.verified_at
              ? format(new Date(details.verified_at), "dd MMM yyyy, hh:mm a")
              : "-"
          }
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <InfoCard
            ...
            value={details.number_of_properties}
          />
          <InfoCard
            icon={Users}
            label="Total Staff"
            value={details.total_staff}
          />
          <InfoCard
            icon={History}
            label="Years of Experience"
            value={`${details.years_of_experience} Years`}
          />
          <InfoCard
            icon={DollarSign}
            label="Annual Turnover"
            value={
              details.annual_turnover
                ? `₹${details.annual_turnover.toLocaleString()}`
                : "-"
            }
          />
          <InfoCard
            ...
            value={details.cities_of_operation?.join(", ")}
          />
        </div>
      </div>
        ...
        <div className="space-y-3">
          ...
          <div className="p-4 rounded-xl border border-border/50 bg-muted/10 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span
                className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${details.has_warehouse ? "bg-green-500/10 text-green-500" : "bg-red-500/10 text-red-500"}`}
              >
        ...
        <div className="space-y-3">
          ...
          <div className="p-4 rounded-xl border border-border/50 bg-muted/10 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span
                className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${details.has_showroom ? "bg-green-500/10 text-green-500" : "bg-red-500/10 text-red-500"}`}
              >
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span
            className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${details.has_delivery_vehicles ? "bg-green-500/10 text-green-500" : "bg-red-500/10 text-red-500"}`}
          >
            {details.has_delivery_vehicles
              ? `Yes (${details.number_of_vehicles || 0})`
              : "No"}
          </span>
        </div>
        {details.has_delivery_vehicles &&
          details.vehicle_details?.length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
              {details.vehicle_details.map((v: any, idx: number) => (
                <div
                  key={idx}
                  className="p-3 rounded-lg border border-border/50 bg-card/50 flex flex-col gap-2"
                >
