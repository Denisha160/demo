...
import { listInterestedProducts, addInterestedProduct, removeInterestedProduct } from '@/services/api';
import { InterestedProductsApiResponse, InterestedProduct } from '@/types/interestedProducts';
...
export function useLeadInterestedProducts(leadId: string) {
    return useQuery<InterestedProduct[]>({
        ...
        queryFn: async () => {
            if (!leadId) return [];
            const response = await listInterestedProducts(leadId) as InterestedProductsApiResponse;
...
export function useAddInterestedProduct() {
    ...
    return useMutation({
        ...
        onSuccess: (response: ApiResponse<unknown>, variables) => {
            queryClient.invalidateQueries({ queryKey: ['lead-interested-products', variables.leadId] });
