...
const GenerateSerialsPage = () => {
  ...
  useEffect(() => {
    ...
    const newPreview = Array.from(
      { length: Math.min(watchQuantity, 500) },
      (_, i) => {
  ...
  const onUpdateSerial = (id: string, newSerial: string) => {
    setPreviewSerials((prev) =>
      prev.map((s) => (s.id === id ? { ...s, serial_number: newSerial } : s)),
    );
  };
    ...
    sync(payload, {
      onSuccess: (res) => {
        ...
        const message =
          ...
          "Serials synchronized successfully";
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-muted-foreground font-mono mt-0.5">
              {urlBatchId
                ? `Synchronizing serial numbers for batch ${selectedBatch?.batch_number || ""}`
                : "Assign unique identifiers to batch units"}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
                ...
                <div className="grid grid-cols-2 gap-4 p-3 bg-muted/20 border border-dashed border-border/60 rounded-sm animate-in fade-in slide-in-from-top-2">
                  <div className="space-y-1">
                    ...
                    <p className="text-xs font-bold">
                      {selectedBatch.initial_quantity} Units
                    </p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-[10px] text-muted-foreground uppercase">
          ...
          <div className="bg-card border border-border rounded-lg p-5 shadow-sm">
            ...
            <div className="mt-4 p-3 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/40 rounded-sm flex items-start gap-3">
              ...
              <p className="text-[10px] text-amber-800 dark:text-amber-400">
                ...
                <span className="font-bold underline">{watchQuantity}</span>{" "}
                serial numbers for{" "}
                <strong>
                  {selectedBatch?.batch_number || "selected batch"}
                </strong>
            ...
            <div className="space-y-4 flex-1">
              <div className="p-3 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/40 rounded-sm flex items-start gap-3">
                ...
                <p className="text-[10px] text-amber-800 dark:text-amber-400">
                  ...
                  numbers below before final generation. Maximum quantity for
                  preview is 500 items.
                </p>
              </div>
