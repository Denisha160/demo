...
const BomModal = ({ isOpen, onClose, bomId, isViewOnly = false }: BomModalProps) => {
    ...
    const { data: finishedGoods = [], isLoading: isLoadingFG } = useProductsCombobox({
        type: 'FINISHED_GOOD',
        status: 'active',
        search: debouncedFgSearch.trim() || undefined
    });
    const { data: rawMaterials = [], isLoading: isLoadingRM } = useProductsCombobox({
        type: 'RAW_MATERIAL',
        status: 'active',
    ...
    const isPending = isCreating || isUpdating || isLoadingBOM || isLoadingFinishedProduct || isLoadingFG || isLoadingRM;
        ...
        if (rawMaterials.length > 0) {
            setMaterials(prev => {
                ...
                const updatedMaterials = prev.map(m => {
                    const product = rawMaterials.find((p: Product) => p.id === m.raw_product_id);
                    if (product && (m.cost_price !== product.cost_price || m.product_name !== product.product_name)) {
                        hasChanges = true;
                        return {
                            ...m,
    ...
    const handleMaterialChange = useCallback((index: number, field: keyof RawMaterialItem, value: string | number) => {
        setMaterials(prev => {
            ...
            if (field === "raw_product_id") {
                ...
                const finalValue = (currentRawQuantity === "" as unknown as number || Number(currentRawQuantity) === 0) ? 1 : currentRawQuantity;

                // Check if this product is already added in another row
                const existingIndex = newMaterials.findIndex((m, i) => i !== index && m.raw_product_id === value);

                if (existingIndex !== -1) {
                    // Item already exists -> Merge quantity and remove current row
                    const currentQuantityToMerge = Number(finalValue) || 1;
                    const existingQuantity = Number(newMaterials[existingIndex].raw_quantity) || 0;
                    ...
                    setMaterialErrors(prevErrors => prevErrors.filter((_, i) => i !== index));
    ...
    return (
        <Modal
            ...
            title={isEditing ? "Edit Manufacturing Recipe" : isViewOnly ? "Recipe Details" : "Create New Recipe"}
            description={isEditing || isViewOnly
                                        ...
                                        <tr key={index} className="hover:bg-accent/50 transition-colors">
                                            <td className="p-2 px-3">
                                                <Combobox
                                                    ...
                                                    onValueChange={(val) => handleMaterialChange(index, "raw_product_id", val)}
                                                    placeholder="Select Material"
                                                    searchPlaceholder="Search..."
                                                    emptyText={isLoadingRM ? "Loading..." : "No products."}
                                                ...
                                                {materialErrors[index]?.raw_product_id && (
                                                    <p className="text-[9px] text-destructive mt-0.5 font-medium px-1">{materialErrors[index]?.raw_product_id}</p>
                                                )}
                                            </td>
                                            <td className="p-2 px-3">
                                                <Input
                                                    type="number"
                                                    value={m.raw_quantity === 0 && !isEditing ? "" : m.raw_quantity}
                                                    onChange={(e) => {
                                                        const val = e.target.value;
                                                        handleMaterialChange(index, "raw_quantity", val === "" ? "" : parseFloat(val));
                                                    }}
                                                ...
                                                {materialErrors[index]?.raw_quantity && (
                                                    <p className="text-[9px] text-destructive mt-0.5 font-medium text-center">{materialErrors[index]?.raw_quantity}</p>
                                                )}
                                            </td>
                                            <td className="p-2 px-3 text-center">
                                                <Badge variant="outline" className="text-[9px] font-bold rounded-sm h-6 px-2 bg-muted/30">
                                                    {m.raw_unit}
                                                </Badge>
                                            </td>
                                            <td className="p-2 px-3 text-right font-mono font-medium text-primary">
                                                ₹{((Number(m.raw_quantity) || 0) * (m.cost_price || 0)).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                            </td>
                                            <td className="p-2 px-3 text-center">
                                                {!isViewOnly && (
                                                    <Button
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="p-4 bg-primary/5 rounded-md border border-primary/10 space-y-2 group hover:bg-primary/[0.08] transition-all">
                        <div className="flex items-center gap-2 text-primary">
                            ...
                            <span className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wide">Base Material Cost</span>
                        </div>
                        <div className="flex flex-col">
                            <span className="text-xl font-bold font-mono tracking-tight text-primary">₹{totalCost.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                            <span className="text-[10px] text-muted-foreground font-medium italic font-semibold uppercase tracking-wide">Per Unit of Finished Product</span>
                        </div>
                    </div>
                    ...
                    <div className="p-4 bg-muted/30 rounded-md border border-border space-y-2 group hover:bg-muted/40 transition-all">
                        <div className="flex items-center gap-2 text-muted-foreground">
                            ...
                            <span className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wide">Planned Selling Price</span>
                        </div>
                        <div className="flex flex-col">
                            <span className="text-xl font-bold font-mono tracking-tight text-foreground">₹{sellingPrice.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                            <span className="text-[10px] text-muted-foreground font-medium italic font-semibold uppercase tracking-wide">From Product Settings</span>
                        </div>
                    </div>

                    <div className={`p-4 rounded-md border transition-all space-y-2 group ${profit >= 0 ? 'bg-emerald-500/5 border-emerald-500/10 hover:bg-emerald-500/10' : 'bg-destructive/5 border-destructive/10 hover:bg-destructive/10'}`}>
                        <div className={`flex items-center gap-2 ${profit >= 0 ? 'text-emerald-600' : 'text-destructive'}`}>
                            <TrendingUp className={`h-4 w-4 group-hover:-translate-y-0.5 transition-transform ${profit < 0 ? 'rotate-180' : ''}`} />
                            <span className="text-[10px] font-semibold uppercase tracking-wide">Est. Margin ({margin.toFixed(1)}%)</span>
                        </div>
                        <div className="flex flex-col">
                            <span className={`text-xl font-bold font-mono tracking-tight ${profit >= 0 ? 'text-emerald-600' : 'text-destructive'}`}>
                                {profit >= 0 ? '+' : ''}₹{profit.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                            </span>
                            <span className="text-[10px] text-muted-foreground font-medium italic font-semibold uppercase tracking-wide">Projected Profit per Unit</span>
                        </div>
                    </div>
                </div>
            </div>
