...
const getExpiryCell = (expiryDate: string | null) => {
  if (!expiryDate)
    return (
      <span className="text-muted-foreground/50 text-xs italic">No expiry</span>
    );
  const days = differenceInDays(new Date(expiryDate), new Date());
  if (isPast(new Date(expiryDate)))
    return <span className="text-xs text-destructive font-bold">Expired</span>;
  if (days <= 30)
    return (
...
const BatchesPage = () => {
  ...
  const expiredCount = items.filter(
    (b) =>
      b.status === "expired" ||
      (b.expiry_date && isPast(new Date(b.expiry_date))),
  ).length;
    ...
    <div className="w-full mx-auto space-y-4 animate-fade-in pb-10">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <SummaryCard
          ...
          className={
            expiredCount > 0 ? "border-destructive/20 bg-destructive/5" : ""
          }
        />
        <SummaryCard
          label="Depleted"
