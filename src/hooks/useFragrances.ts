...
export const useFragranceList = (params?: Record<string, unknown>) => {
    return useQuery<FragranceListResponse>({
        ...
        queryFn: async () => {
            const response = await listFragrance(params) as ApiResponse<FragranceListResponse>;
...
export const useFragranceCombobox = (params?: Record<string, unknown>) => {
    return useQuery<Fragrance[]>({
        ...
        queryFn: async () => {
            const response = await listFragrance({ ...params, combobox: true }) as ApiResponse<FragranceComboboxResponse>;
...
export const useCreateFragrance = () => {
    ...
    return useMutation<ApiResponse<unknown>, Error, FragranceCreatePayload>({
        ...
        onError: (error) => {
            ...
            const message = apiError.response?.data?.message || apiError.message || 'Failed to create fragrance';
            toast.error(message);
        },
    });
};
...
export const useUpdateFragrance = () => {
    ...
    return useMutation<ApiResponse<unknown>, Error, FragranceUpdatePayload>({
        mutationFn: async ({ id, ...payload }) => {
            const response = await updateFragrance(id, payload) as ApiResponse<unknown>;
            return response;
        },
        onSuccess: (response) => {
            queryClient.invalidateQueries({ queryKey: queryKeys.fragrances.all });
        ...
        onError: (error) => {
            const apiError = error as ApiError;
            const message = apiError.response?.data?.message || apiError.message || 'Failed to update fragrance';
            toast.error(message);
        },
    });
};
...
export const useDeleteFragrance = () => {
    ...
    return useMutation<ApiResponse, Error, string>({
        ...
        onError: (error) => {
            const apiError = error as ApiError;
            const message = apiError.response?.data?.message || apiError.message || 'Failed to delete fragrance';
            toast.error(message);
        },
    });
};
